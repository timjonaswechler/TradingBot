...
pub enum PivotKind { High, Low, Both }
...
impl PivotDetector {
    pub fn new(left: usize, right: usize, kind: PivotKind) -> Self {
        ...
        Self { left, right, kind, buf: VecDeque::with_capacity(left + right + 1) }
    }

    fn window(&self) -> usize { self.left + self.right + 1 }
}
...
impl RollingDetector for PivotDetector {
    fn on_candle(&mut self, c: &Candle, bar: u64) -> Option<AnchorEvent> {
        ...
        if self.buf.len() < self.window() { return None; }
        ...
        for (i, (_, other)) in self.buf.iter().enumerate() {
            if i == mid { continue; }
            if is_high && other.high >= cand.high { is_high = false; }
            if is_low  && other.low  <= cand.low  { is_low  = false; }
            if !is_high && !is_low { break; }
        }

        // Tie-break: if both match (flat bar), prefer High — unlikely in practice.
        if is_high {
            return Some(AnchorEvent::PivotHigh { bar: cand_bar, price: cand.high, volume: cand.volume });
        }
        if is_low {
            return Some(AnchorEvent::PivotLow  { bar: cand_bar, price: cand.low,  volume: cand.volume });
        }
        None
    }
}
...
mod tests {
    ...
    fn candle(h: f64, l: f64) -> Candle {
        Candle { timestamp: 0, symbol: "T".into(), open: l, high: h, low: l, close: (h + l) / 2.0, volume: 1.0, timeframe: "1m".into() }
    }
    ...
    fn detects_pivot_low() {
        ...
        for (i, &l) in lows.iter().enumerate() {
            ...
            let c = Candle { timestamp: 0, symbol: "T".into(), open: l, high: l + 10.0, low: l, close: l, volume: 1.0, timeframe: "1m".into() };
            if let Some(ev) = det.on_candle(&c, i as u64) { hits.push(ev); }
        }
        assert_eq!(hits.len(), 1);
        match hits[0] {
    ...
    fn equal_highs_do_not_form_pivot() {
        ...
        let mut hits = 0;
        for (i, &h) in highs.iter().enumerate() {
            if det.on_candle(&candle(h, h - 0.5), i as u64).is_some() { hits += 1; }
        }
        assert_eq!(hits, 0);
    }
}
