...
pub fn adx(candles: &[Candle], period: usize) -> Option<AdxResult> {
    ...
    // DI series
    let di_plus: Vec<f64>  = apdm.iter().zip(atr.iter()).map(|(p, t)| 100.0 * p / t).collect();
    let di_minus: Vec<f64> = andm.iter().zip(atr.iter()).map(|(n, t)| 100.0 * n / t).collect();

    // DX series
    let dx: Vec<f64> = di_plus.iter().zip(di_minus.iter()).map(|(p, n)| {
        let sum = p + n;
        if sum == 0.0 { 0.0 } else { 100.0 * (p - n).abs() / sum }
    }).collect();
...
mod tests {
    ...
    fn insufficient_data() {
        let c: Vec<Candle> = (1..=27).map(|i| candle(i as f64 + 1.0, i as f64 - 1.0, i as f64)).collect();
        assert_eq!(adx(&c, 14), None); // needs 28
    }

    #[test]
    fn computes_without_panic() {
        let c: Vec<Candle> = (1..=60).map(|i| candle(i as f64 + 1.0, i as f64 - 1.0, i as f64)).collect();
    ...
    fn strong_uptrend_high_adx() {
        let c: Vec<Candle> = (1..=60).map(|i| candle(i as f64 + 0.5, i as f64 - 0.5, i as f64)).collect();
        let r = adx(&c, 14).unwrap();
        assert!(r.adx > 25.0, "ADX {:.1} should indicate strong trend", r.adx);
