...
mod tests {
    ...
    fn insufficient_data() {
        let c: Vec<Candle> = (1..=14).map(|i| candle(i as f64 + 0.5, i as f64 - 0.5, i as f64, 100.0)).collect();
        assert_eq!(mfi(&c, 14), None); // needs 15
    }

    #[test]
    fn all_up_days_returns_100() {
        let c: Vec<Candle> = (1..=15).map(|i| candle(i as f64 + 0.5, i as f64 - 0.5, i as f64, 100.0)).collect();
        assert_eq!(mfi(&c, 14), Some(100.0));
    }

    #[test]
    fn value_in_range() {
        let c: Vec<Candle> = [1.0, 2.0, 1.5, 3.0, 2.5, 4.0, 3.5, 5.0, 4.5, 6.0, 5.5, 7.0, 6.5, 8.0, 7.5]
            .iter().enumerate().map(|(i, &p)| candle(p + 0.5, p - 0.5, p, (i + 1) as f64 * 100.0)).collect();
