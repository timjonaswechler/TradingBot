...
pub use live_position_type::LivePosition;
    ...
    fn args_bsatn(&self) -> Result<Vec<u8>, __sats::bsatn::EncodeError> {
        match self {
            ...
            Reducer::OpenPaperPosition{
                ...
                entry_base_price: entry_base_price.clone(),
                entry_effective_fill_price: entry_effective_fill_price.clone(),
                entry_spread_adjustment: entry_spread_adjustment.clone(),
                entry_fixed_fee: entry_fixed_fee.clone(),
                entry_percent_fee: entry_percent_fee.clone(),
                entry_total_cost: entry_total_cost.clone(),
}),
            ...
            Reducer::RecordPaperPositionClosed{
                ...
                entry_percent_fee: entry_percent_fee.clone(),
                entry_total_cost: entry_total_cost.clone(),
                exit_base_price: exit_base_price.clone(),
                exit_effective_fill_price: exit_effective_fill_price.clone(),
                exit_spread_adjustment: exit_spread_adjustment.clone(),
                exit_fixed_fee: exit_fixed_fee.clone(),
                exit_percent_fee: exit_percent_fee.clone(),
                exit_total_cost: exit_total_cost.clone(),
                gross_pnl: gross_pnl.clone(),
                total_costs: total_costs.clone(),
                net_realized_pnl: net_realized_pnl.clone(),
}),
            Reducer::UpdatePaperPositionRiskBoundaries{
                open_projection_key,
                strategy_identity,
    ...
    fn try_from(raw: __ws::v2::TransactionUpdate) -> Result<Self, Self::Error> {
        ...
        for table_update in __sdk::transaction_update_iter_table_updates(raw) {
            match &table_update.table_name[..] {
                "candles" => db_update
                    .candles
                    .append(candles_table::parse_table_update(table_update)?),
                "live_positions" => db_update
                    .live_positions
                    .append(live_positions_table::parse_table_update(table_update)?),
...
impl __sdk::DbUpdate for DbUpdate {
    fn apply_to_client_cache(
        ...
        diff.candles = cache
            .apply_diff_to_table::<Candle>("candles", &self.candles)
            .with_updates_by_pk(|row| &row.id);
        diff.live_positions = cache
            .apply_diff_to_table::<LivePosition>("live_positions", &self.live_positions)
            .with_updates_by_pk(|row| &row.id);
...
impl<'r> __sdk::AppliedDiff<'r> for AppliedDiff<'r> {
    fn invoke_row_callbacks(
        ...
        callbacks.invoke_table_row_callbacks::<Candle>("candles", &self.candles, event);
        callbacks.invoke_table_row_callbacks::<LivePosition>(
            "live_positions",
            &self.live_positions,
            event,
        );
