...
- `domain/`
  ...
  - Legacy non-domain helpers are carried only as transitional code until follow-up cleanup issues remove or migrate them; the legacy executor helper was removed in #110, the legacy signal/decision vocabulary was removed in #111, and the legacy context helper was removed in #112.
...
- `domain/src/position.rs` — passive value types that should remain in future `domain`: `PositionSide`, `OpenPosition`, `ClosedPosition`, and `PositionRiskBoundaries`. `PositionSide` keeps passive formatting/serde helpers only. `OpenPosition` should contain `PositionSide`, Runtime Asset/symbol, entry price, `quantity`, entry time, and grouped `risk_boundaries` Position Risk Boundaries. `ClosedPosition` contains the closed `OpenPosition`, exit price, exit time, and realized PnL as a result value. Entry Risk Parameters are opening-decision inputs, while an Open Position carries current Position Risk Boundaries. These values must stay field/data-only: no PnL/equity helpers, no close/open/update methods, no stop/take-profit validation or evaluation, and no Portfolio Transition behavior. Runtime-specific Portfolio Snapshot, Portfolio Transition, PnL/equity calculation, Entry Risk validation, Position Risk Update validation, and Risk Exit semantics belong in `trading-runtime`.
- `domain/src/signal.rs` — removed in #111. The legacy decision/signal vocabulary (`Signal` / `TradeDecision`) is no longer exported from `domain`; the canonical target is `trading-runtime/src/decision.rs` (`StrategyDecision` plus typed Rhai `decision::*`). Do not re-add legacy strategy semantics or compatibility mapping here.
...
- `domain/src/executor.rs` — removed in #110. The legacy `domain::Action`, `domain::plan_action`, and public `domain::realized_pnl` helper are no longer exported; canonical Execution Planning is `trading-runtime::plan_execution` over `StrategyDecision`, and canonical PnL/equity calculation remains internal to `trading-runtime/src/portfolio.rs`. Realized PnL remains visible as runtime output (for example `ClosedPosition.realized_pnl` / Runtime Events), but PnL/equity formulas should not be exposed as public `domain` helper APIs.
