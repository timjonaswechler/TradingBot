...
- #84 — Anchored v2 / Market Structure redesign. Keep this as a human/design issue, not an implementation or splitting task yet. Re-validate the current `anchored` concept before adding Fibonacci or Secondary-Timeframe structure outputs. Market Structure is a runtime-owned derived domain: it is derived from Market State, exposed to Rhai through Market View, and emitted for future explanation through DB-free Structure Annotations/Runtime output. Rhai strategies may read Market Structure and express selection/filtering rules, but persistent Structure Object truth, lifecycle, and annotations remain owned by `trading-runtime`, not Strategy State, DB, runners, or UI code. The target Rhai API direction is an explicit Market Structure surface rather than continuing `anchored` as the long-term strategy-authoring term. The accepted declaration direction is `structure_config()` as the single explicit declaration surface: do not auto-discover top-level Structure declarations for #84, and do not require duplicate top-level handles plus registration. Strategy-author examples should prefer one returned namespaced registry, e.g. `let s = structure_config::new(); let swing = s.points.pivots("swing", 3, 3); s.objects.trendline("resistance", swing).with_side(...); s`. Point/object IDs inside this hook are declarations/references, not a second registration path; multiple `structure_config::new()` registries inside one hook should not be magically merged. Strategy reads should use `market.structure.active("object_id")` for declared Structure Object IDs; an unknown ID should be an error, while a declared but currently inactive object should return no active snapshots. If post-constructor fluent methods such as `.with_left_bars(...)` are kept, the design must avoid copy-vs-mutation ambiguity; otherwise prefer constructor arguments for required point parameters. The design needs to distinguish Market Structure Points, Structure Anchors, active Structure Objects, and append-only Structure Annotations so strategies can use current active structure while future UI/backtest explanation can reconstruct the historical chart objects. Avoid silent overwrite-only semantics for structure objects unless paired with historical annotations/events. ADR 0001 may need revision or supersession if Fibonacci should move from an automatic anchored evaluator toward explicit Market Structure primitives and strategy-selected anchors. Pine Script may be used as a reference for bar-by-bar structure-object lifecycle only; broader Pine-inspired Rhai strategy-authoring API exploration belongs to #97, not #84.
...
- #97 — Pine-inspired Rhai strategy-authoring API design. Keep this as a human/design issue and separate from #84. Analyze Pine Script as a reference system for bar-by-bar strategy authoring, but produce a Rhai-native typed/fluent API proposal aligned with ADR 0005. Do not create a Pine compatibility layer, Pine parser, general Rhai rewrite, or UI/manual drawing behavior.
