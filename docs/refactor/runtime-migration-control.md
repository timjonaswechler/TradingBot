...
- #81 — Strategy State v2. V1 primitive, session-local Strategy State is intentional, not a missing feature. Keep this as a human decision issue until runtime-backed live/backtest strategies reveal real needs; do not add arrays/maps/host objects or restart persistence opportunistically. Any persistence decision must preserve live/backtest/restart parity and clarify whether storage belongs in Trading Runtime, runner IO, or a future strategy-state store.
...
- #83 — Market View candle history snapshot benchmark. This issue is measurement-first: benchmark/inspect the current per-Strategy-Tick Rhai Market View snapshot cost and document whether optimization is needed. Do not implement caching, bounded histories, or borrowed views in #83 unless the issue is explicitly re-scoped after measurement; any optimization design must preserve Rhai safety and runtime ownership.
- #84 — Anchored v2. Treat this as a planning/splitting issue, not one broad implementation task. Split Primary-timeframe anchored Fibonacci from Secondary-Timeframe anchored/structure outputs. Primary anchored Fibonacci can become an agent-ready child slice under ADR 0001 once its minimal typed config/output contract is explicit. Secondary-Timeframe anchored outputs need human/design-first work because they affect timeframe configuration, Market View access, readiness, warmup, and missing/stale output semantics.
