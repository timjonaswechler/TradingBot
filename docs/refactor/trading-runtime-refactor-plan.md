...
- Runtime-local Portfolio State uses realized-cash semantics: opening a position does not subtract/reserve notional from cash balance; closing a position applies realized PnL to cash balance. When the #42 Execution Cost Model is enabled, runtime-owned simulated fill accounting may adjust effective fill prices and apply per-fill fees to runtime-local realized cash, while still not adding notional reservation, buying-power, margin, or account-reconciliation behavior. More realistic buying-power, margin, and reservation behavior belongs to later account snapshot / portfolio-coordinator work.
